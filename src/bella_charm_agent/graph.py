...
def _match_step_answer(
    ...
    result: StepAnswerMatch = matcher.invoke(
        ...
        "literal match. Customers often answer indirectly: naming a "
        "product instead of literally saying 'yes', naming a specific "
        "example of a category, answering only part of a multi-part "
        "question, or a plain 'no'/'yes' that clearly picks a side of an "
        "either/or question. Use judgment about what they most likely "
        "mean in context, and return the exact matching option string "
        "verbatim. But don't force it: if they're describing something "
        "genuinely different from every listed option, leave "
        "matched_option null rather than guessing the closest-sounding "
        "one -- e.g. saying they'll pick the order up in person when the "
        "choices are 'UK'/'International' delivery (that's neither), or "
        "naming a specific, real, distinctly-named product that isn't "
        "actually one of the listed options, even if it shares a visual "
        "or thematic trait with one (e.g. 'Claddagh' is its own named "
        "design, not an instance of 'Heart', even though it "
        "traditionally features a heart motif)."
    )
